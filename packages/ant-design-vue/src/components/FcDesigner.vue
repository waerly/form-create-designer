                    ...
                    <a-modal v-model:open="preview.state" width="80%" class="_fd-preview-dialog"
                        ...
                        <template v-if="previewStatus === 'form'">
                            ...
                            <div style="text-align: center; margin-top: 10px;">
                                <a-button type="primary" ghost @click="previewGetData">{{ t('form.getData') }}</a-button>
                            </div>
                        </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const methods = {
            ...
            previewGetData() {
                if (data.preview.api && data.preview.api.formData) {
                    const formData = data.preview.api.formData();
                    console.log("formData,",formData)
                    data.previewData.json = JSON.stringify(formData, null, 2);
                    data.previewData.state = true;
                }
            },
            openPreview() {
